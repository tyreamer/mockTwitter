...
function humanNumbers($num) {
  ...
  return $x_display;
	...
	function UpdateLocation ($name) {
		...
		$sql = mysql_query("Select location_id from location where location_name = '".$name."'", $this->connection);		
		
		//IF not, create one
		if (!$sql ||  mysql_num_rows($sql) <= 0) 
		{		
			$sql = "Insert into location (location_name) Values ('".$name."')";
		...
		$sql = "Update person set location = '".$name."' where id_user = ".$user_rec['id_user']."";
	...
	function GetFBLink($userid) {
		 if(!$this->GetUserFromEmail($this->UserEmail(),$user_rec))
        {
        }  
		
		$link = mysql_query("Select fbLink from person where id_user = '".$userid."' ", $this->connection);		
				
		if (!$link ||  mysql_num_rows($link) <= 0) {		
			
			return false;
		}
			
		$userRow = mysql_fetch_assoc($link);
	
        return $userRow['fbLink'];
	}
	...
	function SearchUsers($key) {
		...
		if ($key == '') {
			...
			$result = mysql_query("Select location_id from location where location_name = '".$key."'",$this->connection);  
			
			if(!$result || mysql_num_rows($result) <= 0)
			{
					//Find all people (by name and location) related to this search ordered by followers
	...
	function GetTotalComments($post_id) {
		...
		$myQuery = mysql_query("SELECT count(comment_id) as totalcomments FROM comment where post_id = '". $post_id ."'");
	...
	function UpdateCollectionImage($cid, $img) {
	  ...
	  {			
			...
			//Get current Image and owner
			$collectionQry = mysql_query("Select collection_image, collection_owner from collection where collection_id= '" . $cid. "'");  									
					
			while ($row = mysql_fetch_assoc($collectionQry))
			{				
				//make sure this is our collection
				if ($userID == $row['collection_owner']) 
				{	
					$update_query = mysql_query('Update collection SET collection_image = "images/'.$userID.'/'. $imageName . '" WHERE collection_id = "' . $cid . '"', $this->connection);
					...
					$this->imageResizing($path, $imageName, 500, 500);
					
					//Remove the old photo
					unlink($row['collection_image']);				
				}
			}
 ...
 public function checkhashSSHA($salt, $password) {
    ...
    function CollectRegistrationSubmission(&$formvars)
    {
        ...
        $formvars['fbLink'] = $this->Sanitize($_POST['fbLink']);
    ...
    function SendUserConfirmationEmail(&$formvars)
    {
		...
		"Use your Connect Name to Login: ".$formvars['connectName']."\r\n\r\n".
    ...
    function InsertIntoDB(&$formvars)
    {
		...
		"' . $confirmcode . '",
		"    0  ",
		"'.$this->SanitizeForSQL($formvars['fbLink']).'"
		)';  
