...
$conn = new mysqli($servername, $username, $password, $db);	
		...
		if (isset($_POST['user'])) {
			...
			if (isset($_POST['sort']) && $_POST['sort'] != '' && $_POST['sort'] != undefined) {
					...
					$qry = "Select follower_user_id from follower where user_id= $user LIMIT 12 OFFSET $offset";
					
			}
			else 
			{		
				$qry = "Select follower_user_id from follower where user_id=$user ORDER BY user_id DESC LIMIT 12";
			}
		...
		for ($j = 0; $j < sizeOf($allFollowers); $j++) {
				...
				$cName  = $fgmembersite->GetConnectName($allFollowers[$j]['follower_user_id']);
